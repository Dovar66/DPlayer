...
public class PhoneUtil {
    ...
    public static String getIMEI(Context context1) {
        ...
        if (!TextUtils.isEmpty(SharePreferenceUtil.getUniqueId(context))) {
            return SharePreferenceUtil.getUniqueId(context);
        } else {
            if (!TextUtils.isEmpty(((TelephonyManager) context.getSystemService(context.TELEPHONY_SERVICE)).getDeviceId())) {
                //判断是否全是0
                if (!((TelephonyManager) context.getSystemService(context.TELEPHONY_SERVICE)).getDeviceId().equals("000000000000000")) {
                    SharePreferenceUtil.setUniqueId(context, ((TelephonyManager) context.getSystemService(context.TELEPHONY_SERVICE)).getDeviceId());
                    return ((TelephonyManager) context.getSystemService(context.TELEPHONY_SERVICE)).getDeviceId();
                } else {
                    if (!TextUtils.isEmpty(Settings.Secure.getString(context.getContentResolver(), Settings.Secure.ANDROID_ID))) {
                        SharePreferenceUtil.setUniqueId(context, Settings.Secure.getString(context.getContentResolver(), Settings.Secure.ANDROID_ID));
                        return Settings.Secure.getString(context.getContentResolver(), Settings.Secure.ANDROID_ID);
                    } else {
                        SharePreferenceUtil.setUniqueId(context, getUniquePsuedoID());
    ...
    public static boolean isMIUI() {
        if (SharePreferenceUtil.getSystemName(MyApplication.getInstance()).equals(SharePreferenceUtil.SYSTEM_XIAOMI)) {
            return true;
        }
        ...
        if (isMIUI) {
            SharePreferenceUtil.saveSystemName(MyApplication.getInstance(), SharePreferenceUtil.SYSTEM_XIAOMI);
        } else {
            SharePreferenceUtil.saveSystemName(MyApplication.getInstance(), "");
    ...
    public static boolean isEMUI() {
        if (SharePreferenceUtil.getSystemName(MyApplication.getInstance()).equals(SharePreferenceUtil.SYSTEM_HUAWEI)) {
            return true;
        }
        ...
        if (isEMUI) {
            SharePreferenceUtil.saveSystemName(MyApplication.getInstance(), SharePreferenceUtil.SYSTEM_HUAWEI);
        } else {
            SharePreferenceUtil.saveSystemName(MyApplication.getInstance(), "");
    ...
    public static boolean isMeizuFlyme() {
        if (SharePreferenceUtil.getSystemName(MyApplication.getInstance()).equals(SharePreferenceUtil.SYSTEM_MEIZU)) {
            return true;
        }

        boolean isFlyme = getMeizuFlymeOSFlag().toLowerCase().contains("flyme");
        if (isFlyme) {
            SharePreferenceUtil.saveSystemName(MyApplication.getInstance(), SharePreferenceUtil.SYSTEM_MEIZU);
        } else {
            SharePreferenceUtil.saveSystemName(MyApplication.getInstance(), "");
